...
let M_inverse_array = [[ 3.243063328, -1.538376194, -0.49893282],
                       [-0.968963091,  1.875424508,  0.041543029],
                       [ 0.055683923, -0.204174384,  1.057994536]];

let A_prime_array = [[0.001368, 0.004243, 0.01431,  0.04351, 0.13438, 0.2839,	0.34828, 0.3362,  0.2908, 0.19536, 0.09564, 0.03201, 0.0049, 0.0093, 0.06327, 0.1655,  0.2904, 0.43345, 0.5945,	0.7621,	0.9163,	 1.0263, 1.0622, 1.0026,  0.85445, 0.6424,  0.4479,  0.2835, 0.1649, 0.0874, 0.04677, 0.0227,  0.011359, 0.00579,  0.002899, 0.00144],
...
let W_array = [0.499755, 0.546482, 0.827549, 0.91486, 0.934318, 0.866823, 1.04865, 1.17008, 1.17812, 1.14861, 1.15923, 1.08811, 1.09354, 1.07802, 1.0479, 1.07689, 1.04405, 1.04046, 1, 0.963342, 0.95788, 0.886856, 0.900062, 0.895991, 0.876987, 0.832886, 0.836992, 0.800268, 0.802146, 0.822778, 0.782842, 0.697213, 0.716091, 0.74349, 0.61604, 0.698856];

let M_inverse = new Matrix(M_inverse_array, 3, 3, "values");
let A_prime = new Matrix(A_prime_array, 3, 36, "values");
let W = new Matrix(W_array, 1, 36, "value");

let omega = A_prime.dotProduct(A_prime_array[1], W_array);

let T = new Matrix(M_inverse.multiply(A_prime).outputArray(), M_inverse.rows, A_prime.columns, "values").multiply(new Matrix(W_array, 36, 36, "diag")).multiply(1/omega);

function LHTSS(T, sRGB) {
    ...
    let rgb = convert_sRGB_to_RGB(sRGB);
    ...
    while(count < maxit) {
        ...
        let F_b = d1.multiply(T.transpose()).multiply(lambda);
        let F_c = T.multiply(d0);
        let F = F_a.add(F_b).appendBottom(F_c.subtract(rgb));     // 39x1 F vector

        let J_a = new Matrix(d2.multiply(T.transpose()).multiply(lambda).transpose().outputArray(), 36, 36, "diag");
        let J_b = d1.multiply(T.transpose());
        let J_c = T.multiply(d1).appendRight(new Matrix(0, 3, 3, "fill"));
        J = D.add(J_a).appendRight(J_b).appendBottom(J_c);     // 39x39 J matrix
        let delta = systemSolver(J, F.multiply(-1)); // Solve Newton system of equations J*delta = -F
...
function convert_sRGB_to_RGB(sRGB) {
...
function convert_RGB_to_sRGB(rgb) {
